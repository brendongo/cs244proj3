...
class RipLController(object):
  ...
  def _flood(self, event):
    ...
    #print "PACKET: {} [{}]".format(packet, dir(packet))
    #for sw in self._raw_dpids(t.switches()):
    #    sw_name = t.id_gen(dpid=sw).name_str()
    #    for sport, (dst, _) in t.ports[sw_name].iteritems():
    #        if sw != dpid or (sw == dpid and sport != in_port):
    #            print "SW: {} sending from port: {}, data: {}".format(sw, sport, event.data)
    #            self.switches[sw].send_packet_data(sport, event.data)
