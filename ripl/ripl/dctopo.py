...
class JellyFishTop(StructuredTopo):
    LAYER_EDGE = 2
    LAYER_HOST = 3
    ...
    def __init__(self):
        ...
        super(JellyFishTop, self).__init__(None, None)
        self.id_gen = ID
        self.node_info = defaultdict(dict)
        #leftHost = self.addHost( 'h1' , **self.def_nopts(self.LAYER_HOST, "h1"))
        #rightHost = self.addHost( 'h2', **self.def_nopts(self.LAYER_HOST, "h2"))
        #leftSwitch = self.addSwitch( 's3', **self.def_nopts(self.LAYER_EDGE, "s3"))
        #rightSwitch = self.addSwitch( 's4', **self.def_nopts(self.LAYER_EDGE, "s4"))

        ## Add links d
        #self.addLink( leftHost, leftSwitch , bw=1000)
        #self.addLink( leftSwitch, rightSwitch , bw=1000)
        ...
        for switch in tqdm(graph.vertices()):
            ...
            self.addSwitch(
                switch_name, **self.def_nopts(self.LAYER_EDGE, switch_name))
            for i in xrange(SERVERS_PER_SWITCH):
                host_name = "h{}".format(host_index)
                host_index += 1
                server = self.addHost(
    ...
    def def_nopts(self, layer, name):
        ...
        if layer == self.LAYER_HOST:
          ...
          d.update({'mac': id.mac_str()})
        d.update({'dpid': "%016x" % id.dpid})
        self.node_info[name]['layer'] = layer
        return d
